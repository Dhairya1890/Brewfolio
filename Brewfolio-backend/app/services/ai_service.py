...
async def generate_profile_content(
    ...
    try:
        ...
        if "429" in error_msg or "quota" in error_msg or "exhausted" in error_msg or "rate limit" in error_msg:
            logger.warning(f"Google API rate limit hit ({e}). Trying OpenRouter fallbacks...")
            ...
            for or_model in FALLBACK_MODELS:
                try:
                    ...
                    if completion.choices:
                        logger.info(f"Successfully generated profile using OpenRouter fallback model: {or_model}")
                        return extract_json_from_response(completion.choices[0].message.content or "")
                except Exception as fallback_err:
                    err_str = str(fallback_err).lower()
                    if "429" in err_str or "rate limits" in err_str:
                        logger.warning(f"OpenRouter model {or_model} rate limited, trying next...")
                        continue
                    else:
                        logger.error(f"OpenRouter generation failed on {or_model}: {fallback_err}")
                        continue
            
            logger.error("All OpenRouter free fallback models failed (likely due to upstream rate limits).")
        else:
            logger.error(f"Google AI generation failed immediately: {e}")

        # Return a fallback structure
        return {
            "name": raw_data.github.get("name", "Developer") if isinstance(raw_data.github, dict) else "Developer",
